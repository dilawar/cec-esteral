...
void m2blif(sm* m, ostream &o)
{
    ...
    for(j=0; j<m->no_ff; j++)
    {
        ...
        */
        ...
        // hold
        o<<"11";
        for(i=0; i<m->no_states; i++) if(m->enc_table[i][j]=='1') o<<"-";
        o<<" 1\n";
        // normal op
...
int main(int argc, char *argv[])
{
    ...
    if(argc != 3 )
    {
        cerr <<"Usage: "<<argv[0]<<"smfile bliffile\n";
        exit(-1);
    }
    ifstream smfile(argv[1]);
    if(!smfile.good()) exit(-1);
    ofstream bliffile(argv[2]);
    if(!bliffile.good()) exit(-1);
